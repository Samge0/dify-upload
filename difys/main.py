...
import glob
import os
from difys import api, configs, difydb
...
if __name__ == '__main__':
    ...
    for i in range(file_total):
        ...
        if difydb.exist_name(filename):
            doc_item = difydb.get_doc_item_by_name(filename)
            timeutils.print_log(f"{file_path} 已上传，跳过")
            continue
        
        # 文件不存在，上传文件=>切片=>解析并等待解析完毕
        timeutils.print_log(f'{file_path}，开始上传&切片并等待解析完毕')
        status = api.upload_file_with_check(
            file_path=file_path, 
            kb_id=configs.DIFY_DOC_KB_ID
        )
        
        timeutils.print_log(file_path, "上传&切片状态：", status)
